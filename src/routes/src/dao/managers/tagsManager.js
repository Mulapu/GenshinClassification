...
class tagsManager extends manager {
    ...
    async modifyTag( obj ) { // Obj contain: tagName, newTagName, newTagDescription and imageBuffer
        const tagInfo = await this.findTag( obj?.tagName ) // tagInfo returns the entire object

        if ( !tagInfo ) return 'Must provide a valid tag name to modify. Tag not found.'

        const imageUrl = await ( async function () { // Modify the image on cloudinary
            if ( !obj?.imageBuffer ) return null
            
            const currentUrl = tagInfo?.image 
            if ( currentUrl ) removeImage( currentUrl ) // Remove image if exists
            const imageBuffer = obj?.imageBuffer
            const image = await uploadImage ( imageBuffer )
            return image.secure_url
        })()
        ...
        const updateFields = {
            ...( obj?.newTagName && { name: obj.newTagName } ),
            ...( obj?.newTagDescription && { description: obj.newTagDescription } ),
            ...( imageUrl && { image: imageUrl } )
        };

        const request = await this.update( tagInfo._id, { $set: updateFields } )
        return request
    }
}
