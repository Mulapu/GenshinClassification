...
create.addEventListener( 'click', async function ( e ) {
    ...
    const json = JSON.stringify( { tagName: 'aaa', newTagName: 'aa', newTagDescription: 'asdfasdf' } )
    const dataForm = new FormData()
    dataForm.append( 'tagName', 'aa' )
    dataForm.append( 'newTagName', 'aaa' )
    dataForm.append( 'newTagDescription', 'desc' )
    dataForm.append( 'image', image.files[0] )

    const data = await apiFetch( 'POST', 'tags/modify', dataForm )
